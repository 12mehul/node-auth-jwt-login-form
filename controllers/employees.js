  ...
  try {
    ...
    const mailOptions = {
      ...
      html:
        "<p> Hii " +
        name +
        ', Please copy the link and <a href = "http://localhost:3003/api/employees/reset-password?token = ' +
        token +
...
const getUser = async (req, res) => {
  try {
    ...
    const user = await Employee.findOne({ _id: claims._id });
...
const forgetPassword = async (req, res) => {
  try {
    ...
    if (userData) {
      ...
      res.status(200).send({
        success: true,
        msg: `http://localhost:3003/api/employees/reset-password?token=${randomString}`,
      });
    } else {
      res
        .status(200)
